...
3. Both `train_dtln.py` and `train_cruse.py` have a default batch size as specified in their respective papers referenced in the thesis document. `-b 10` is used as an example because bigger batch sizes could cause an error with the example dataset since it only contains 50 audio files and a train/validation split is by default set to 80/20. With a bigger dataset, bigger batch_sizes will be okay as well.
